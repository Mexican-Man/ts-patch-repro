...
export const iterate_metadata_collection = (collection) => {
...
const isArrayRecursive = (visited) => (array) => (meta) => {
    if (visited.has(meta))
        return false;
    visited.add(meta);
    return (meta.arrays.some((a) => a === array || isArrayRecursive(visited)(array)(a.value)) ||
        meta.aliases.some((alias) => isArrayRecursive(visited)(array)(alias.value)) ||
        meta.tuples.some((t) => !t.recursive &&
...
const isTupleRecursive = (visited) => (tuple) => (meta) => {
    if (visited.has(meta))
        return false;
    visited.add(meta);
    return (meta.tuples.some((t) => t === tuple ||
        t.elements.some((e) => isTupleRecursive(visited)(tuple)(e))) ||
        meta.arrays.some((a) => !a.recursive && isTupleRecursive(visited)(tuple)(a.value)) ||
...
const isObjectRecursive = (visited) => (obj) => (meta) => {
    ...
    return (meta.objects.some((o) => obj === o ||
        ...
        meta.arrays.some((array) => !array.recursive &&
            isObjectRecursive(visited)(obj)(array.value)) ||
        meta.tuples.some((tuple) => !tuple.recursive &&
            tuple.elements.some((elem) => isObjectRecursive(visited)(obj)(elem))) ||
