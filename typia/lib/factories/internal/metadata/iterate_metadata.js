...
export const iterate_metadata = (checker) => (options) => (collection) => (meta, type, resolved, aliased) => {
    if (type.isTypeParameter() === true)
        throw new Error(`Error on typia.MetadataFactory.generate(): non-specified generic argument on ${meta.getName()}.`);
    if ((aliased !== true &&
        iterate_metadata_alias(checker)(options)(collection)(meta, type)) ||
        iterate_metadata_intersection(checker)(options)(collection)(meta, type, resolved, aliased) ||
        iterate_metadata_union(checker)(options)(collection)(meta, type, resolved) ||
        iterate_metadata_resolve(checker)(options)(collection)(meta, type, resolved, aliased))
        return;
    iterate_metadata_coalesce(meta, type) ||
        iterate_metadata_constant(checker)(options)(meta, type) ||
        iterate_metadata_template(checker)(options)(collection)(meta, type) ||
