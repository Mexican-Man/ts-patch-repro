...
export const check_array_length = (metaTags) => (input) => metaTags
    .map((tag) => ({
    tag,
    expression: tag.kind === "items"
        ? ts.factory.createStrictEquality(ts.factory.createNumericLiteral(tag.value), input)
        : tag.kind === "minItems"
